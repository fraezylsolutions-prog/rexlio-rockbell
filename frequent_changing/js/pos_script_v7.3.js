(function ($) {
    ...
    function getSelectedOrderDetailsFutureSale(sale_no_new) {
       ...
       * price_tier is deliberately separate from order_type.
       *   order_type  = how the order is fulfilled (1 Regular, 2 Take away, 3 Delivery)
       *                 and is what gates table selection.
       *   price_tier  = which price column applies (1 Regular, 2 Take away,
       *                 3 Delivery, 4 VIP, 5 Club) and gates nothing else.
       * A VIP or Club order is therefore still free to be a dine in order at a table.
       ***********************************************************************/
      let ir_price_tier_map = {
          1: {attr: "data-price",          prop: "price"},
          2: {attr: "data-price_take",     prop: "price_take"},
          3: {attr: "data-price_delivery", prop: "price_delivery"},
          4: {attr: "data-price_vip",      prop: "price_vip"},
          5: {attr: "data-price_club",     prop: "price_club"}
      };

      /* The tier in force right now. An explicit tier selection wins; with none
         set we derive it from the order type button so behaviour is unchanged
         until the tier selector exists. */
      function getSelectedPriceTier(){
          let explicit = Number($("#selected_price_tier").val());
          if(explicit && ir_price_tier_map[explicit]){
              return explicit;
          }
          let selected_button = $(".main_top").find("button[data-selected=selected]").attr("data-id");
          if(selected_button == "take_away_button"){
              return 2;
          }
          if(selected_button == "delivery_button"){
              return 3;
          }
          if(selected_button == "vip_button" || selected_button == "club_button"){
              //read the tier off the button itself so VIP (4) and Club (5) stay distinct
              return Number($(".main_top").find("button[data-selected=selected]").attr("data-tier_key")) || 4;
          }
          return 1;
      }

      /* Price of an item at a tier. source is either a jQuery element carrying
         data-price* attributes or a plain item object from window.items.
         An unset or zero tier price falls back to the Regular price, matching
         what main_screen.php already does when it emits these values, so an item
         with no VIP price sells at its normal price rather than at zero. */
      function resolveItemPrice(source, tier){
          let mapping = ir_price_tier_map[Number(tier)] ? ir_price_tier_map[Number(tier)] : ir_price_tier_map[1];
          let value;
          if(source && source.jquery){
              value = source.attr(mapping.attr);
              if(value === undefined || value === "" || Number(value) === 0){
                  value = source.attr(ir_price_tier_map[1].attr);
              }
          }else if(source){
              value = source[mapping.prop];
              if(value === undefined || value === "" || Number(value) === 0){
                  value = source[ir_price_tier_map[1].prop];
              }
          }
          return value;
      }
      ...
      /* Phase F item 15: zero stock block.
         ...
         cart. Reads window.items rather than the tile's data attributes, so it does
         not have to be threaded through the seven places .single_item markup is
         built - the tile only has to carry its id, which all seven already do.

         Honest about what this is: the POS is offline first and order placement is
         fire and forget, so this is the primary user experience but NOT a guarantee.
         window.items is a page load snapshot and ages while the screen stays open,
         and when genuinely offline it cannot be refreshed at all. The real gate is
         Sale::add_kitchen_sale_by_ajax() server side, which refuses to persist an
         overselling order even if this check was stale or bypassed.

         Two states must not be confused:
           available_units === ''  -> UNCONSTRAINED. The item has no recipe, so stock
                                      can never limit it. Never block.
           available_units === '0' -> genuinely out of stock. Block.
         Reading '' as 0 would block every recipe-less item outright. */
      function ir_find_item(item_id){
          for(let i = 0; i < window.items.length; i++){
              if(window.items[i].item_id == item_id){
                  return window.items[i];
              }
          }
          return null;
      }
      function ir_stock_would_block(item_id, requested_qty){
          let entry = ir_find_item(item_id);
          if(!entry){
              return false; /* unknown item: never invent a block */
          }
          /* F4 policy, already resolved server side into one field */
          if(String(entry.oversale_block) !== "1"){
              return false;
          }
          let available = entry.available_units;
          if(available === undefined || available === null || String(available).trim() === ""){
              return false; /* unconstrained */
          }
          return Number(requested_qty) > Number(available);
      }
      /* Shared refusal, so the message is identical wherever the block fires. */
      function ir_notify_stock_block(item_id){
          let entry = ir_find_item(item_id);
          let name = entry && entry.item_name ? entry.item_name : "";
          let available = entry ? Number(entry.available_units) : 0;
      ...
      /* VIP|Club price tier button.
         ...
         Regular. The order_type derivation sites treat this button as dine in.
         Switching with items in the cart warns first, matching the order type
         buttons, because every line has to be repriced. */
      /* True when this counter pins the tier for this user. The buttons are also
         rendered disabled server-side; this is a second guard so a re-enabled
         button (dev tools, stale DOM) still cannot change the tier. The real
         enforcement is server side in Sale::add_kitchen_sale_by_ajax(). */
      function isPriceTierLocked(){
          return Number($("#locked_price_tier").val()) > 0;
      }

      /* Waiter attribution PIN check.
         A non-Admin attributing an order to somebody other than themselves must enter
         that person's PIN. Waiters never reach this: their waiter field is a fixed
         hidden input, not a dropdown, and the server has already filtered the list to
         self. The server re-checks at order-save time, so this prompt is the courtesy,
         not the control. */
      let waiter_pin_previous_value = '';
      let waiter_pin_pending_value = '';

      function waiterPinCheckRequired(){
          return Number($("#waiter_pin_required").val()) === 1;
      }

      function closeWaiterPinModal(){
          $("#waiter_pin_modal").removeClass("active");
          $(".pos__modal__overlay").fadeOut(200);
          $("#waiter_pin_input").val('');
          $("#waiter_pin_error").text('');
      }

      function revertWaiterSelection(){
          $("select#select_waiter").val(waiter_pin_previous_value).trigger("change.select2");
          waiter_pin_pending_value = '';
          closeWaiterPinModal();
      }

      /* remember what was selected before, so a failed or cancelled check can revert */
      $(document).on("select2:opening focus", "select#select_waiter", function () {
          waiter_pin_previous_value = $(this).val();
      });

      $(document).on("change", "select#select_waiter", function () {
          let selected_waiter_id = $(this).val();
          let own_user_id = $("#own_user_id").val();

          if(!waiterPinCheckRequired() || !selected_waiter_id || selected_waiter_id == own_user_id){
              waiter_pin_previous_value = selected_waiter_id;
              return;
          }

          waiter_pin_pending_value = selected_waiter_id;
          $("#waiter_pin_target_name").text($(this).find("option:selected").text());
          $("#waiter_pin_error").text('');
          $("#waiter_pin_input").val('');
          $("#waiter_pin_modal").addClass("active");
          $(".pos__modal__overlay").fadeIn(200);
          setTimeout(function(){ $("#waiter_pin_input").focus(); }, 150);
      });

      $(document).on("click", "#waiter_pin_cancel,#waiter_pin_cancel_btn", function () {
          revertWaiterSelection();
      });

      $(document).on("click", "#waiter_pin_submit", function () {
          let entered_pin = $("#waiter_pin_input").val();
          if(!entered_pin){
              return false;
          }
          $.ajax({
              url: base_url + "Sale/verifyWaiterPinAjax",
              method: "POST",
              dataType: "json",
              data: { waiter_id: waiter_pin_pending_value, pin: entered_pin },
              success: function (response) {
                  if(response && Number(response.status) === 1){
                      waiter_pin_previous_value = waiter_pin_pending_value;
                      waiter_pin_pending_value = '';
                      closeWaiterPinModal();
                  }else{
                      $("#waiter_pin_error").text(response && response.msg ? response.msg : '');
      ...
      $(document).on("keypress", "#waiter_pin_input", function (e) {
          if(e.which === 13){
              $("#waiter_pin_submit").click();
          }
      });

      $(document).on("click", ".vip_button,.club_button", function (e) {
          e.preventDefault();
          if(isPriceTierLocked()){
              return false;
          }
          let tier_button = $(this);
          let new_tier = Number(tier_button.attr("data-tier_key"));
          if(!new_tier){
              new_tier = 4;
          }
          if(tier_button.attr("data-selected") == "selected"){
              return false;
          }

          let apply_tier = function(){
              $(".main_top").find("button").attr("data-selected", "unselected").removeClass("selected__btn");
              tier_button.attr("data-selected", "selected").addClass("selected__btn");
          ...
          if(total_items_in_cart > 0){
              let tier_alert = $("#price_tier_changing_alert").val();
              swal(
                  {
                      title: warning + "!",
                      text: tier_alert,
      ...
      /* Keep #selected_price_tier in step with the order type buttons, so that
         returning to Regular/Outlet/Delivery after VIP|Club restores their pricing
         instead of leaving the tier stuck on the last explicit choice. */
      $(document).on("click", ".dine_in_button,.take_away_button,.delivery_button", function (e) {
          if(isPriceTierLocked()){
              return false;
          }
          let button_id = $(this).attr("data-id");
          if(button_id == "take_away_button"){
              $("#selected_price_tier").val(2);
          }else if(button_id == "delivery_button"){
              $("#selected_price_tier").val(3);
          }else{
      ...
      function displayOrderList(){
          //cleared here and set again once the IndexedDB cursor is exhausted below
          window.ir_running_orders_rendered = false;
          ...
          objectStore.openCursor(null, 'prev').onsuccess = function(event) {
              ...
              if (cursor) {
                  ...
                  //cursor exhausted, the panel now reflects everything in IndexedDB.
                  //The deep link handler uses this instead of waiting on a timeout.
                  window.ir_running_orders_rendered = true;
              }
          };
      }
  ...
  // purpose of this function is to delete data from sales table by using data from indexeddb
      ...
      function update_split_parent_sale(order_object_split, sale_no,is_ignore='') {
          ...
          let res = getSelectedOrderDetails(sale_no).then(function(data){
              ...
              order_info += '"price_tier":"' + (rowData.price_tier ? rowData.price_tier : getSelectedPriceTier()) + '",';
      ...
      function push_online(){
          ...
          objectStore.openCursor().onsuccess = function(event) {
              ...
              if (cursor) {
                  ...
                  //Rows already pushed are skipped before the payload is touched.
                  if(online_push!==0){
                      cursor.continue();
                      return;
                  }
                  //An unparsable row must not take the queue down with it. Throwing
                  //here kills the cursor callback synchronously, cursor.continue()
                  //never runs, and every later row stays stuck behind it forever.
                  let rowData;
                  try{
                      rowData = JSON.parse(orders);
                  }catch(parse_error){
                      console.warn("push_online: skipping recent_sales row with unparsable order", sales_id, parse_error);
                      cursor.continue();
                      return;
                  }
      ...
      function push_online_sync(){
          ...
          objectStore.openCursor().onsuccess = function(event) {
              ...
              if (cursor) {
                  ...
                  //Same guard as push_online(): a row that will not parse must be
                  //skipped, not allowed to abort the cursor and strand the queue.
                  let rowData;
                  try{
                      rowData = JSON.parse(orders);
                  }catch(parse_error){
                      console.warn("push_online_sync: skipping recent_sales row with unparsable order", sales_id, parse_error);
                      cursor.continue();
                      return;
                  }
                  let sale_no = rowData.sale_no;
 ...
 invoice_print+= ` </table>
      ...
      $(document).on(
        ...
        function (e) {
            ...
            if (total_items_in_cart > 0) {
                swal(
                    ...
                    function () {
                        ...
                        if (this_action.attr("data-id") == "dine_in_button") {
                            ...
                            //Table selection stays available for take away: a customer
                            //can be waiting at a table for a take away order. Only
                            //Delivery disables it. This matches what the order load
                            //paths already do, which previously disagreed with this one.
                ...
                if ($(this).attr("data-id") == "dine_in_button") {
                    ...
                    //Table selection stays available for take away, see the note in
                    //the matching branch above. Only Delivery disables it.
        ...
        function openProductEditModal(parent_id,item_name, id){
            ...
            for (let key1 in foundItems_variations) {
                ...
                let vr01_price_tier = getSelectedPriceTier();
                item_price = parseFloat(resolveItemPrice(foundItems_variations[key1], vr01_price_tier)).toFixed(ir_precision);
                if (vr01_price_tier == 3){
                    //Delivery keeps its own handling: the price comes from the parent
                    //item rather than the variation, and a selected delivery partner
                    //can override it. Left exactly as it was.
            ...
            if (selected_order_type_object.length > 0) {
              ...
              if(status_continue==true){
                  if(is_variation=="Yes") {
                      ...
                      let promo_price_tier = getSelectedPriceTier();
                      item_price = parseFloat(resolveItemPrice($(this), promo_price_tier)).toFixed(ir_precision);
                      if (promo_price_tier == 3){
                          //Delivery only: a selected delivery partner can override the price
                      ...
                      let item_price_tier = getSelectedPriceTier();
                      item_price = parseFloat(resolveItemPrice($(this), item_price_tier)).toFixed(ir_precision);
                      if (item_price_tier == 3){
                          //Delivery only: a selected delivery partner can override the price
                          let arr_item_details = search_by_menu_id(item_id, window.items);
                          let check_dl_person = 1;
                          $(".custom_li").each(function() {
                      ...
                      /* qty_current is what this cart WOULD hold for this item after the
                         click: every existing line for it, plus the one being added.
                         The vendor computed it here (alongside an unused
                         #stock_not_available lookup against a field that does not exist
                         in the view) and never used either - abandoned scaffolding for
                         exactly this feature. It is the right number, so it is used now. */
                      let qty_current = 1;
                      ...
                      if(ir_stock_would_block(item_id, qty_current)){
                          ir_notify_stock_block(item_id);
                          return false;
                      }
    ...
    $(document).on("click", ".edit_item", function () {
        ...
        $("#add_to_cart").html(add_to_cart_pos);
          ...
          if(parent_id){
              ...
              for (let key1 in foundItems_variations) {
                  let item_price = 0;
                  item_price = parseFloat(resolveItemPrice(foundItems_variations[key1], getSelectedPriceTier())).toFixed(ir_precision);
                  let check_status = '';
    ...
    function setMergeTableFinal(objects,sale_no,table_text,merge_tables){
      ...
      //when add to card button is clicked information goes to table of middle to top
        $(document).on("click", "#add_to_cart", function (e) {
            ...
            if(is_variation_product && !status){
                ...
                /* Third and last path that can put stock into the cart: the modal,
                   used for variations and modifiers. Unlike the other two the quantity
                   is typed rather than incremented by one, so the whole requested
                   amount is checked against what the cart already holds. */
                let ir_qty_in_cart = Number(item_quantity);
                $(".single_order_column").each(function () {
                    let qty_counter = Number($(this).find("#item_quantity_table_" + item_id).html());
                    if (qty_counter && qty_counter != "NAN") {
                        ir_qty_in_cart += qty_counter;
                    }
                });
                if (ir_stock_would_block(item_id, ir_qty_in_cart)) {
                    ir_notify_stock_block(item_id);
                    return false;
                }
      ...
      $(document).on(
        ...
        function () {
          ...
          /* Same abandoned scaffolding as the tile click path, put to use: qty_current
             is the cart total for this item after this increment. Guarding the tile
             click alone would leave this "+" open, and the tile click delegates HERE
             for an item already in the cart. */
          let qty_current = 1;
          $(".single_order_column").each(function () {
      ...
      $(document).on("click", ".place_order_operation", function (e) {
          ...
          if(is_login_checker==true){
              ...
              if (selected_order_type_object.length > 0) {
                  ...
                  } else if (selected_order_type_object.attr("data-id") == "dine_in_button" || (selected_order_type_object.attr("data-id") == "vip_button" || selected_order_type_object.attr("data-id") == "club_button")) {
                      //VIP|Club is a price tier, not a fulfilment type: it books as dine in
                      //and gets the same waiter/customer validation as Regular.
                      order_type = 1;
                  ...
                  order_info += '"price_tier":"' + getSelectedPriceTier() + '",';
    ...
    function show_all_items() {
      ...
      setTimeout(function () {
        ...
        for (let key in foundItems) {
            if (foundItems.hasOwnProperty(key)) {
                ...
                if (foundItems[key].parent_id == '0') {
                    searched_category_items_to_show +=
                        '<div class="single_item animate__animated all_item_custom" data-price="' + foundItems[key].price + '"  data-price_take="' + foundItems[key].price_take + '"    data-is_variation="' + foundItems[key].is_variation + '"  data-parent_id="' + foundItems[key].parent_id + '"    data-price_delivery="' + foundItems[key].price_delivery + '"  data-price_vip="' + foundItems[key].price_vip + '"  data-price_club="' + foundItems[key].price_club + '" data-veg_status="' +
                        veg_status +
      ...
      $(document).on("click", "#hold_cart_info", function (e) {
        ...
        if (selected_order_type_object.attr("data-id") == "delivery_button") {
          ...
          //VIP|Club books as dine in, see note above
          order_type = 1;
    ...
    function arrange_info_on_the_cart_to_modify(response) {
     ...
     let edit_content_object = ($(".edit_content_object").html());
      //edit_content_object comes from tbl_sales.self_order_content and is empty
      //for ordinary orders. Parsing an empty string throws and aborts the rest of
      //the POS start up, so only parse when there is valid JSON to parse.
      if(edit_sale_id && edit_content_object && IsJsonString(edit_content_object)){
  ...
  <div>
    ...
    /* Deep link support for the standalone Running Order screen.
       That screen links here as ...?open_sale_no=XXXX. The running order panel is
       rendered from THIS device's IndexedDB, so the card cannot be selected until
       that render has happened. Poll for it, then select it.
       The order is only ever present when it was placed on this device and by the
       logged in user, because displayOrderList() also filters on user id. When it
    ...
    (function openSaleFromDeepLink(){
        ...
        let target_sale_no = decodeURIComponent(deep_link_match[1]);
        let deep_link_attempts = 0;
        /* Poll fast. Normally we stop on the first or second tick: either the card
           is there, or displayOrderList() has finished its IndexedDB cursor and set
           ir_running_orders_rendered, which proves the order is genuinely absent.
           The attempt cap is only a backstop for the case where the panel never
           renders at all (IndexedDB blocked or unavailable). */
        let deep_link_max_attempts = 40; /* 40 x 150ms is about 6 seconds */
        let deep_link_timer = setInterval(function(){
            deep_link_attempts++;
            let target_card = $('.holder .order_details > .single_order[data-sale_no="' + target_sale_no + '"]');
            if(target_card.length){
                clearInterval(deep_link_timer);
                target_card.trigger("click");
