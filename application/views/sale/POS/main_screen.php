...
foreach($food_menus as $single_menus){
    ...
    //VIP and Club follow the same fallback rule as take away and delivery above:
    //an unset or zero tier price falls back to the Regular price, so an item that
    //has no VIP price simply sells at its normal price rather than at zero.
    $sale_price_vip = (isset($single_menus->sale_price_vip) && $single_menus->sale_price_vip && $single_menus->sale_price_vip!='0.00')
        ? $single_menus->sale_price_vip
        : $sale_price;
    $sale_price_club = (isset($single_menus->sale_price_club) && $single_menus->sale_price_club && $single_menus->sale_price_club!='0.00')
        ? $single_menus->sale_price_club
        : $sale_price;
    ...
    /* Only 4 of 197 live products carry a photo, so the grey placeholder filled
       almost the whole grid with the same square. Where there is no real image we
       substitute a drawn one, chosen from beverage_item - the product already
       knows whether it is a drink or a dish, so nothing is guessed from its name.

       Deliberately a different IMAGE rather than different MARKUP: .single_item
       is built in seven places (here, plus six times in pos_script_v7.3.js from
       window.items), and every one of them reads this same value. Changing the
       markup would have meant editing all seven; changing the URL changes none. */
    $ir_has_photo = (file_exists($img_size) && $single_menus->photo != "");
    if ($ir_has_photo) {
...
foreach($menu_categories as $single_category){
    ...
    if($i = 1){
        $cateogry_slide_to_show .= '<li><a href="#" class="category_button" data-cix="'.((int)$single_category->id % 8).'" id="button_category_'.$single_category->id.'">'.getPlanText($single_category->category_name).'</a></li>';
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo base_url(); ?>assets/POS/css/custom_pos.css?v=7.9">
    ...
    <?php /* Theme layer. The POS renders standalone, so it needs its own copy
         of these - it never passes through userHome.php. */ ?>
    <script>
        (function(){try{var t=localStorage.getItem('rexlio_theme');
    ...
    <link rel="stylesheet" href="<?php echo base_url(); ?>frequent_changing/css/rexlio_theme.css?v=7.9.5">
</head>
            ...
            <div class="left_item">
                <div class="header_part_middle">
                    ...
                    <ul class="icon__menu">
                        ...
                        <li>
                            <a href="<?php echo base_url()?>running-orders" class="header_menu_icon  <?php echo escape_output($is_self_order_class) ?>" target="_blank" rel="opener"  data-tippy-content="<?php
                            echo lang('running_order');
                            ?>">
            ...
            <div class="left_item <?php echo escape_output($is_self_order_class) ? 'self_order_mode':'' ?>">
                ...
                <div class="main_middle" style="<?php echo escape_output($is_self_order_class)?'width: 100%;':'' ?>">
                    <div class="main_top">
                        ...
                        <div class="button_holder <?php echo escape_output($is_self_order_class) ?>">
                            <?php
                                /* Counter price tier lock.
                                   ...
                                   other than Regular, the whole order-type row is fixed:
                                   fulfilment type and price tier are both pinned to the
                                   counter's setting and every button is disabled. Locking the
                                   row wholesale (rather than the tier alone) is what keeps a
                                   take-away order from ever being billed at VIP|Club rates.
                                   Admin/Manager get $locked_tier = 0 and full free selection. */
                                $locked_tier = isset($locked_price_tier) ? (int) $locked_price_tier : 0;
                                $is_tier_locked = $locked_tier > 0;
                                $lock_attr = $is_tier_locked ? ' disabled="disabled" title="' . escape_output(lang('price_tier_locked_to_counter')) . '"' : '';
                                ...
                                //for default order type select
                                    ...
                                    if($is_tier_locked){
                                        //only the button matching the counter's tier is selected
                                        $selected = ($locked_tier == 1) ? 'selected' : '';
                                    }
                            ?>
                            <button class="selected__btn_c  <?php echo escape_output($is_self_order_class) ?> dine_in_button" data-id="dine_in_button"
                                data-selected="<?php echo escape_output($selected)?>"<?php echo $lock_attr ?>>
                                <i class="fal fa-table"></i> <?php echo lang('dine'); ?>
                            </button>
                            ...
                            if($is_tier_locked){
                                $selected = ($locked_tier == 2) ? 'selected' : '';
                            }
                            ?>
                            <button class="selected__btn_c  <?php echo escape_output($is_self_order_class) ?> take_away_button" data-selected="<?php echo escape_output($selected)?>" data-id="take_away_button"<?php echo $lock_attr ?>><i class="fal fa-shopping-bag"></i>
                                <?php echo lang('take_away'); ?></button>
                            ...
                            if($is_tier_locked){
                                $selected = ($locked_tier == 3) ? 'selected' : '';
                            }
                            ?>
                            <?php
                            if(!isFoodCourt()):
                            ?>
                                <button class="selected__btn_c  <?php echo escape_output($is_self_order_class) ?> delivery_button" data-selected="<?php echo escape_output($selected)?>" data-id="delivery_button"<?php echo $lock_attr ?>><i class="fal fa-truck"></i>
                                    <?php echo lang('delivery'); ?></button>
                            <?php
                            elseif($this->session->userdata('role') == 'Admin'):
                            ?>
                            ...
                            ?>
                            <?php
                            /* VIP and Club price tier buttons.
                             *
                             * These were previously one combined "VIP|Club" button bound to tier 4,
                             * which meant tier 5 (Club) could never be selected manually - it was
                             * only ever reachable through a counter lock. They are now genuinely
                             * distinct tiers with their own buttons and their own price columns
                             * (sale_price_vip / sale_price_club).
                             *
                             * Behaviour: these set the PRICE TIER only. order_type stays dine-in,
                             * so table selection remains available exactly as for Regular. The
                             * order_type derivation sites in pos_script treat BOTH buttons as
                             * dine-in for that reason - if a further tier button is ever added it
                             * must be added there too, or the order will record order_type = 0.
                             *
                             * LAYOUT: this row is a CSS grid with a hardcoded column count
                             * (style2.css). It is now repeat(6, 1fr) for these six buttons. A
                             * seventh button will wrap the row and push the footer action bar off
                             * screen - see PHASE_D_PRICING_TIERS.md section 9 before adding one.
                             */
                            ?>
                            <button class="selected__btn_c <?php echo escape_output($is_self_order_class) ?> vip_button" data-id="vip_button" data-selected="<?php echo escape_output($is_tier_locked && $locked_tier == 4 ? 'selected' : '') ?>" data-tier_key="4"<?php echo $lock_attr ?>><i class="fal fa-star"></i>
                                <?php echo lang('vip'); ?></button>
                        ...
                        <?php
                        /* price_tier is carried by the order type buttons themselves rather
                           than a separate row of tier pills. #selected_price_tier is the single
                           source of truth that Stage 3's resolveItemPrice() reads; every button
                           in the group above sets it on click. */
                        $preselected_tier = isset($default_price_tier) && $default_price_tier ? (int) $default_price_tier : 1;
                        //at a locked counter the tier is pinned, not merely pre-selected
                        if($is_tier_locked){
                            $preselected_tier = $locked_tier;
                        }
                        ?>
                        <input type="hidden" id="selected_price_tier" value="<?php echo escape_output($preselected_tier) ?>">
                        <input type="hidden" id="locked_price_tier" value="<?php echo escape_output($locked_tier) ?>">
                        <?php /* 1 = this user must PIN-verify a waiter other than themselves (Admin = 0) */ ?>
                        <input type="hidden" id="waiter_pin_required" value="<?php echo escape_output(!empty($waiter_pin_required) ? 1 : 0) ?>">
                        <input type="hidden" id="own_user_id" value="<?php echo escape_output($this->session->userdata('user_id')) ?>">
                        <?php /* Phase F item 15: refusal text for the client stock block, so the
                                 message is translatable rather than hardcoded in the script. */ ?>
                        <input type="hidden" id="ir_stock_blocked_msg" value="<?php echo escape_output(lang('stock_blocked_item')) ?>">
                        <input type="hidden" id="counter_default_price_tier" value="<?php echo escape_output($preselected_tier) ?>">
                        <?php if($is_self_order=="Yes" && $is_online_order!="Yes"):?>
                    ...
                    <div class="waiter_customer">
                        <div class="left_item <?php echo escape_output($is_self_order_class) ?> <?php echo isFoodCourt()?'isFoodCourt':''?>">
                                <?php
                                /* Order attribution.
                                   A waiter may only attribute an order to themselves, so they get
                                   the same fixed hidden field the Waiter App path already uses
                                   rather than a dropdown. Sale::POS() has also filtered the waiter
                                   list down to self, so no other waiter's name or id is present in
                                   the page at all - this is a restriction, not a disabled control.
                                   Everyone else keeps the normal dropdown; picking somebody other
                                   than themselves triggers the PIN prompt (Admin exempt). */
                                if(isset($is_waiter_user) && $is_waiter_user):
                                ?>
                                ...
                                endif; //close the waiter-user branch
                                ?>
                    ...
                    <div class="main_center <?php echo escape_output($is_self_order_class) ? 'self_order_mode': '' ?> <?php echo escape_output($mobile_responsive_checker_online_order) ? 'irp_online_order': '' ?> <?php echo escape_output($mobile_responsive_checker_self_order) ? 'irp_self_order': '' ?>">
                        <div class="order_table_holder">
                            ...
                            <!-- jump to the newest line on a long order -->
                            <button type="button" id="ir_cart_to_bottom" title="Go to the last item"><i class="fal fa-arrow-down"></i></button>
...
<div class="cus_pos_modal cus_pos_modal_self_online_sale_modal" id="customModalSelfOnlineOrder">
    ...
    <!-- Waiter PIN verification modal.
         Shown when a non-Admin attributes an order to somebody other than themselves.
         The selection is only committed once the server confirms the PIN; on failure or
         cancel the dropdown reverts to its previous value. -->
    <div id="waiter_pin_modal" class="modal">
        <div class="modal-content" id="modal_waiter_pin">
            <h1 class="ir_pos_relative">
                <?php echo lang('enter_waiter_pin'); ?> <span id="waiter_pin_target_name"></span>
                <a href="javascript:void(0)" class="alertCloseIcon" id="waiter_pin_cancel">
                    <i class="fal fa-times"></i>
                </a>
            </h1>
            <div class="waiter_pin_body">
                <input type="password" id="waiter_pin_input" class="form-control" inputmode="numeric"
                       maxlength="4" autocomplete="off" placeholder="****">
                <p class="waiter_pin_error" id="waiter_pin_error"></p>
            </div>
            <div class="waiter_pin_actions">
                <button type="button" id="waiter_pin_submit" class="bg__green"><?php echo lang('submit'); ?></button>
                <button type="button" id="waiter_pin_cancel_btn" class="bg_pos_cancel"><?php echo lang('cancel'); ?></button>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript" src="<?php echo base_url(); ?>frequent_changing/js/pos_script_v7.3.js?v=6.5"></script>
    ...
    /*This variable could not be escaped because this is building object*/
    /* Phase F item 14: each entry carries `available_units` - how many more of that
       item current stock allows, computed server side (ledger closing balance minus
       committed, minimised across the recipe) for THIS outlet only.

       Convention, and F5 depends on it: '' means UNCONSTRAINED (the item has no
       recipe, so stock can never limit it) while '0' means genuinely out of stock.
       Reading '' as zero would block every recipe-less item outright.

       Snapshot at page load. The POS is offline first, so this ages while the screen
       stays open; the server check at order placement is the real gate.

       Phase F item 17: each entry also carries `oversale_block` - '1' when running
       out of stock should stop the sale, '0' when it should not. Already RESOLVED
       server side from the global default and the per item override, so F5 reads one
       field and never re-derives the rule in JS. The same oversaleBlocked() helper
       backs the server check, so the two cannot drift apart. */
