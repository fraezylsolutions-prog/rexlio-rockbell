...
/**
 * Counter price tier lock (Phase D).
 *
 * A counter can be assigned a default_price_tier. When that tier is anything other
 * than 1 (Regular), the whole order-type row on the sale screen is locked for
 * ordinary staff: fulfilment type AND price tier are both fixed to the counter's
 * setting. Locking the whole row rather than the tier alone avoids the situation
 * where a take-away order at a VIP|Club counter would be billed at VIP|Club rates.
 *
 * Counters left on tier 1 behave exactly as before: no lock, free selection.
 */
if (!function_exists('priceTierOverrideRoleNames')) {
    function priceTierOverrideRoleNames() {
        //Roles allowed to override a counter's locked tier. Matched case-insensitively
        //against tbl_roles.role_name. 'Admin' is handled separately via tbl_users.role.
        return array('Manager', 'Leader', 'Sales Supervisor');
    }
}
if (!function_exists('hasPriceTierOverride')) {
    /**
     * @return bool TRUE if this user may switch tier/order type at a locked counter
     */
    function hasPriceTierOverride() {
        $CI = &get_instance();
        $role = $CI->session->userdata('role');
        ...
        if (isset($row->role_name) && $row->role_name) {
            foreach (priceTierOverrideRoleNames() as $privileged_role) {
...
if (!function_exists('orderTypeForPriceTier')) {
    /**
     * Fulfilment type implied by a price tier. VIP|Club (4) and Club (5) are pricing
     * concepts that book as ordinary dine-in orders.
     * @return int
     */
    function orderTypeForPriceTier($tier) {
        $tier = (int) $tier;
        if ($tier == 2) { return 2; }
        if ($tier == 3) { return 3; }
        return 1;
...
/**
 * Register gate for order persistence (base-product fix).
 *
 * The vendor build only checked the register in Sale::__construct(), i.e. when the
 * sale SCREEN loads. The endpoints that actually write orders had no check at all,
 * so a tab left open when a register closed carried on saving sales, and a direct
 * POST bypassed the gate entirely. This is the server-side enforcement.
 *
 * Rule matches the UI gate: waiters need ANY open register at their outlet;
 * everyone else needs their own. Food court and online-order flows keep the
 * vendor's exemptions.
 *
 * @return bool TRUE when an order may be written right now
 */
if (!function_exists('canPersistOrderNow')) {
    function canPersistOrderNow() {
        $CI = &get_instance();
        //preserve the vendor's exemptions verbatim
        if ($CI->session->has_userdata('is_online_order') == "Yes") {
            return TRUE;
        }
        if (isFoodCourt()) {
            return TRUE;
        ...
        if ($designation == "Waiter") {
            $CI->db->select('id')->from('tbl_register')
                   ->where('outlet_id', $outlet_id)->where('register_status', 1);
            return $CI->db->get()->num_rows() > 0;
        }
        $CI->load->model('Common_model');
        return $CI->Common_model->isOpenRegister($CI->session->userdata('user_id'), $outlet_id) != 0;
    }
}

/**
 * Order attribution helpers (waiter selection integrity).
 *
 * Two rules:
 *   - A waiter may only attribute an order to themselves. The sale screen renders a
 *     fixed hidden field instead of a dropdown for them, and Sale::POS() filters the
 *     waiter list to self so no other waiter's data reaches the browser.
 *   - Anyone else (Cashier, Manager, ...) attributing an order to someone OTHER than
 *     themselves must enter that person's 4 digit login_pin first. Admin is exempt,
 *     consistent with how Admin bypasses the other gates in this system.
 *
 * The PIN is verified server side and the result is remembered in the session, so the
 * order-save path can re-check it. A DOM edit that swaps the waiter id does not survive
 * that second check.
 */
if (!function_exists('isWaiterUser')) {
    function isWaiterUser() {
        $CI = &get_instance();
        if ($CI->session->userdata('designation') == "Waiter") {
            return TRUE;
        }
        return $CI->session->userdata('is_waiter') == "Yes";
    }
}
if (!function_exists('waiterPinCheckRequired')) {
    /**
     * @return bool TRUE when this user must supply a PIN to attribute an order to
     *              somebody else. Admin is exempt; waiters can only pick themselves
     *              so it never triggers for them in practice.
     */
    function waiterPinCheckRequired() {
        $CI = &get_instance();
        return $CI->session->userdata('role') != "Admin";
    }
}
if (!function_exists('markWaiterPinVerified')) {
    function markWaiterPinVerified($waiter_id) {
        $CI = &get_instance();
        $verified = $CI->session->userdata('verified_waiter_ids');
        if (!is_array($verified)) {
            $verified = array();
        }
        if (!in_array((int) $waiter_id, $verified)) {
            $verified[] = (int) $waiter_id;
        }
        $CI->session->set_userdata('verified_waiter_ids', $verified);
    }
}
if (!function_exists('isWaiterAttributionAllowed')) {
    /**
     * Server-side gate used at order save time.
     * @return bool
     */
    function isWaiterAttributionAllowed($waiter_id) {
        $CI = &get_instance();
        $waiter_id = (int) $waiter_id;
        if (!$waiter_id) {
            return TRUE; //no waiter attributed, nothing to verify
        }
        if (!waiterPinCheckRequired()) {
            return TRUE; //Admin
        }
        if ($waiter_id == (int) $CI->session->userdata('user_id')) {
            return TRUE; //attributing to self never needs a PIN
        }
        $verified = $CI->session->userdata('verified_waiter_ids');
        return is_array($verified) && in_array($waiter_id, $verified);
    }
}

/**
 * Outlet access and sales-enablement helpers (Phase F).
 *
 * Two separate concerns, deliberately kept apart:
 *
 *   canEnterOutlet()      - MAY this user work in this outlet at all?
 *   isSalesEnabledOutlet() - MAY anyone sell from this outlet, whoever they are?
 *
 * The first is per user, the second is a property of the outlet. Central Store
 * uses both: it is assigned only to Admin/Manager/Accountant, and it has
 * is_sales_enabled = 0 so that even an Admin standing in it cannot take an order.
 */
if (!function_exists('canEnterOutlet')) {
    /**
     * BASE-PRODUCT FIX: Outlet::setOutletSession() set the outlet session with no
     * check that the user was assigned to that outlet, and Custom::encrypt_decrypt()
     * is a no-op passthrough, so the "encrypted" id in the URL is the raw id. Any
     * user holding enter-67 could therefore enter ANY outlet in the company by
     * typing the URL, regardless of their tbl_users.outlets assignment.
     *
     * Read from tbl_users rather than the session copy: session_outlets is
     * snapshotted at login, so an assignment revoked mid-session would otherwise
     * still be honoured until the user logged out.
     *
     * @param int $outlet_id
     * @return bool TRUE when this user is allowed into that outlet
     */
    function canEnterOutlet($outlet_id) {
        $CI = &get_instance();
        $outlet_id = (int) $outlet_id;
        if (!$outlet_id) {
            return FALSE;
        }
        //The outlet must actually exist and belong to this company. Without this,
        //setOutletSession() walks on to $outlet_details->id against a NULL row,
        //which is a fatal on PHP 8. Checked for everyone including Admin, since
        //Admin is precisely who could reach an arbitrary id.
        $company_id = (int) $CI->session->userdata('company_id');
        $CI->db->select('id')->from('tbl_outlets')
               ->where('id', $outlet_id)->where('del_status', 'Live');
        if ($company_id) {
            $CI->db->where('company_id', $company_id);
        }
        $exists = $CI->db->get();
        ...
        //Admin sees every outlet, matching Common_model::getAllOutlestByAssign()
        ...
        $user = $CI->db->select('outlets')->from('tbl_users')
                       ->where('id', $user_id)->where('del_status', 'Live')
                       ->get()->row();
        if (!isset($user->outlets) || $user->outlets === NULL || $user->outlets === '') {
            return FALSE;
        }
        //tbl_users.outlets is a CSV, and carries stray empty entries in places
        //(Outlet::addEditOutlet appends with a leading comma), so filter rather
        //than trusting a bare in_array on the split result.
        foreach (explode(',', $user->outlets) as $assigned) {
            if ((int) trim($assigned) === $outlet_id) {
                return TRUE;
            }
        }
        return FALSE;
...
if (!function_exists('stockClosingBalance')) {
    /**
     * Closing stock balance for one item, in CONSUMPTION units (Phase F).
     *
     * This formula was written out by hand in four places: getAlertCount() here,
     * Inventory_model::getCurrentInventory(), and the inventory and alert list
     * views. Four copies of an eleven term expression is how figures drift between
     * screens, so it now lives once and every caller uses it.
     *
     * Input is a row from any of the stock queries in Inventory_model, all of which
     * expose the same sum aliases. Quantities held in PURCHASE units (purchases,
     * transfers, production) are scaled by conversion_rate; consumption, waste and
     * adjustments are already in consumption units and are not.
     *
     * @param  object $row
     * @return float
     */
    function stockClosingBalance($row) {
        $n = function ($v) { return is_numeric($v) ? (float) $v : 0.0; };
        $conv = $n(isset($row->conversion_rate) ? $row->conversion_rate : 0);

        return ($n(isset($row->total_purchase) ? $row->total_purchase : 0) * $conv)
             -  $n(isset($row->total_consumption) ? $row->total_consumption : 0)
             -  $n(isset($row->total_modifiers_consumption) ? $row->total_modifiers_consumption : 0)
             -  $n(isset($row->total_waste) ? $row->total_waste : 0)
             +  $n(isset($row->total_consumption_plus) ? $row->total_consumption_plus : 0)
             -  $n(isset($row->total_consumption_minus) ? $row->total_consumption_minus : 0)
             + ($n(isset($row->total_transfer_plus) ? $row->total_transfer_plus : 0) * $conv)
             - ($n(isset($row->total_transfer_minus) ? $row->total_transfer_minus : 0) * $conv)
             + ($n(isset($row->total_transfer_plus_2) ? $row->total_transfer_plus_2 : 0) * $conv)
             - ($n(isset($row->total_transfer_minus_2) ? $row->total_transfer_minus_2 : 0) * $conv)
             + ($n(isset($row->total_production) ? $row->total_production : 0) * $conv);
    }
}
if (!function_exists('oversaleBlocked')) {
    /**
     * Should selling this food menu be blocked once its stock runs out? (Phase F item 17)
     *
     * Two settings resolve here and nowhere else, so client and server can never
     * disagree about the rule:
     *
     *   tbl_food_menus.oversale_policy  0 = use global default
     *                                   1 = always allow  (never block this item)
     *                                   2 = always block
     *   tbl_companies.allow_oversale    'Yes' = oversale permitted (do not block)
     *                                   'No'  = oversale not permitted (block)
     *
     * Note the polarity: allow_oversale = 'Yes' means "selling past zero is fine",
     * so it resolves to NOT blocked. Both columns default to the permissive value,
     * which makes the F4 migration a behavioural no-op.
     *
     * F5 consumes this in two places - the pre-computed `oversale_block` field in
     * window.items, and the server check in add_kitchen_sale_by_ajax().
     *
     * VARIATIONS DO NOT INHERIT YET - deferred, but the decision is already made.
     * A variation is a tbl_food_menus row with parent_id > 0 and it draws on the
     * PARENT's stock, not its own inventory. So inheritance must be one-directional
     * and the parent's block must WIN: if the parent blocks, the variation blocks
     * regardless of its own setting; otherwise the variation's own policy applies.
     * That is deliberately stronger than a plain "fall back to the parent value" -
     * a variation set to Always Allow must not be able to keep selling stock the
     * parent has already run out of.
     *
     * The whole policy map is loaded once per request and memoised, so calling this
     * inside the food-menu loop that builds window.items costs one query, not N.
     *
     * @param  int $food_menu_id
     * @return bool TRUE when running out of stock should stop the sale
     */
    function oversaleBlocked($food_menu_id) {
        $CI = &get_instance();
        static $cache = array();

        $company_id = (int) $CI->session->userdata('company_id');
        if (!isset($cache[$company_id])) {
            $entry = array('global_blocks' => FALSE, 'per_item' => array());

            //Fail OPEN on a missing column, same reasoning as isSalesEnabledOutlet():
            //a half applied migration must never start blocking sales on its own.
            $company = $CI->db->select('allow_oversale')->from('tbl_companies')
                              ->where('id', $company_id)->get();
            if ($company) {
                $row = $company->row();
                if (isset($row->allow_oversale)) {
                    //Only an explicit 'No' blocks. Testing for "not Yes" instead would
                    //make an empty or unrecognised value start blocking sales, which is
                    //the wrong failure direction and asymmetric with Setting::index(),
                    //which stores anything that is not 'No' as 'Yes'.
                    $entry['global_blocks'] = (strcasecmp(trim($row->allow_oversale), 'No') == 0);
                }
            }

            $menus = $CI->db->select('id, oversale_policy')->from('tbl_food_menus')
                            ->where('company_id', $company_id)->get();
            if ($menus) {
                foreach ($menus->result() as $menu) {
                    $entry['per_item'][$menu->id] = (int) $menu->oversale_policy;
                }
            }
            $cache[$company_id] = $entry;
        }

        $policy = isset($cache[$company_id]['per_item'][$food_menu_id])
            ? $cache[$company_id]['per_item'][$food_menu_id]
            : 0;
        ...
        return $cache[$company_id]['global_blocks']; //use global default
    }
}
if (!function_exists('stockBlockedItems')) {
    /**
     * Which of the requested items cannot be sold for lack of stock? (Phase F item 15)
     *
     * THE SERVER SIDE GATE. Given the items on an order, returns the names of those
     * whose requested quantity exceeds what is available at this outlet, honouring
     * the F4 oversale policy. An empty array means the order may proceed.
     *
     * WHAT THIS CAN AND CANNOT DO - stated here rather than only in the docs, because
     * anyone reading this function needs to know it. The POS is offline first and
     * push_online_for_kitchen() is fire and forget: the order is written to IndexedDB
     * and drawn on screen BEFORE this endpoint is called, and the response only
     * surfaces as a toast. So refusing here does not un-take the order - it refuses to
     * PERSIST it server side. The client check is the real user experience; this is
     * the integrity backstop that a tampered payload or a stale tab cannot get past.
     * When genuinely offline, neither runs against fresh data. This is the same
     * limitation the register gate has, and it is not fixable without changing the
     * IndexedDB architecture.
     *
     * The rule, matching the client exactly:
     *   - oversale not blocked for this item  -> never refuse
     *   - item has no recipe (unconstrained)  -> never refuse
     *   - otherwise refuse when requested qty > available units
     *
     * @param  array  $items           objects with ->food_menu_id and ->qty
     * @param  int    $outlet_id       the selling outlet
     * @param  int    $company_id
     * @param  string $exclude_sale_no order being modified, excluded from committed
     * @return array  names of items that cannot be fulfilled
     */
    function stockBlockedItems($items, $outlet_id, $company_id, $exclude_sale_no = '') {
        $CI = &get_instance();
        if (empty($items)) {
            return array();
        }
        //aggregate first: the same food menu can appear on several lines of one order,
        //and each line on its own might pass while the total does not.
        $requested = array();
        foreach ($items as $item) {
            if (!isset($item->food_menu_id)) {
                continue;
            }
            $id = (int) $item->food_menu_id;
            $qty = isset($item->qty) ? (float) $item->qty : 0;
            if (!$id || $qty <= 0) {
                continue;
            }
            //an item whose policy allows oversale can never fail, so it costs nothing
            //to drop it here and keep the stock queries smaller
            if (!oversaleBlocked($id)) {
                continue;
            }
            $requested[$id] = (isset($requested[$id]) ? $requested[$id] : 0) + $qty;
        }
        if (!$requested) {
            return array();
        }
        ...
        $scope = array((int) $outlet_id);
        $on_hand = array();
        foreach ($CI->Inventory_model->getInventory('', '', '', $scope) as $row) {
            if (isset($row->id) && $row->id) {
                $on_hand[$row->id] = stockClosingBalance($row);
            }
        }
        $committed = $CI->Inventory_model->getCommittedStock($scope, $exclude_sale_no);
        $available = array();
        foreach ($on_hand as $ingredient_id => $qty) {
            $available[$ingredient_id] = $qty - (isset($committed[$ingredient_id]) ? $committed[$ingredient_id] : 0);
        }
        $requirements = $CI->Inventory_model->getRecipeRequirements($company_id);

        $blocked = array();
        foreach ($requested as $food_menu_id => $qty) {
            $recipe = isset($requirements[$food_menu_id]) ? $requirements[$food_menu_id] : array();
            $sellable = maxSellableUnits($recipe, $available);
            //NULL means unconstrained: no recipe, so stock can never limit it.
            //Treating NULL as zero here would refuse every recipe-less item.
            if ($sellable === NULL) {
                continue;
            }
            if ($qty > $sellable) {
                $blocked[] = getFoodMenuNameById($food_menu_id);
            }
        }
        return $blocked;
    }
}
if (!function_exists('maxSellableUnits')) {
    /**
     * How many more units of a food menu could be sold from available stock (Phase F).
     *
     * The binding constraint is whichever recipe item runs out first, so this is the
     * MINIMUM across the recipe of floor(available / qty consumed per unit).
     *
     * Returns NULL for "unconstrained", which is a real and important state, not a
     * missing value: a food menu with no recipe rows consumes nothing measurable, so
     * stock can never limit it. F5 must treat NULL as "never block" rather than as
     * zero - reading it as zero would block every recipe-less item outright.
     *
     * @param  array $requirements ingredient_id => qty consumed per unit
     * @param  array $available    ingredient_id => available quantity
     * @return int|null            units sellable, or NULL when unconstrained
     */
    function maxSellableUnits($requirements, $available) {
        if (empty($requirements)) {
            return NULL;
        }
        $limit = NULL;
        foreach ($requirements as $ingredient_id => $per_unit) {
            $per_unit = (float) $per_unit;
            //a zero or negative requirement cannot constrain anything, and would
            //divide by zero if it were allowed through
            if ($per_unit <= 0) {
                continue;
            }
            $have = isset($available[$ingredient_id]) ? (float) $available[$ingredient_id] : 0.0;
            $units = (int) floor($have / $per_unit);
            if ($units < 0) {
                $units = 0; //already oversold; no negative "sellable" figure
            }
...
if (!function_exists('getAccessibleOutletIds')) {
    /**
     * The outlets this user may see data for, as an array of ints (Phase F).
     *
     * This is the scope of "All Outlets" on the stock screens. It deliberately
     * means "every outlet YOU can reach", not "every outlet in the company":
     * a combined view that reached past a user's assignment would give back
     * exactly what the canEnterOutlet() fix took away.
     *
     * Read from tbl_users, not session_outlets, for the same reason as
     * canEnterOutlet(): the session copy is snapshotted at login and would keep
     * honouring an assignment that has since been revoked.
     *
     * @return array list of outlet ids, possibly empty
     */
    function getAccessibleOutletIds() {
        $CI = &get_instance();
        $company_id = (int) $CI->session->userdata('company_id');
        if (!$company_id) {
            return array();
        ...
        $CI->db->select('id')->from('tbl_outlets')
               ->where('company_id', $company_id)->where('del_status', 'Live')
               ->order_by('id', 'ASC');
        ...
        if ($CI->session->userdata('role') != "Admin") {
            ...
            $assigned = array();
            if (isset($user->outlets) && $user->outlets !== NULL && $user->outlets !== '') {
                foreach (explode(',', $user->outlets) as $one) {
                    $one = (int) trim($one);
                    if ($one) {
                        $assigned[] = $one;
                    }
            ...
            //rebuild the select, where_in has to be applied to this query object
            $CI->db->select('id')->from('tbl_outlets')
                   ->where('company_id', $company_id)->where('del_status', 'Live')
                   ->where_in('id', $assigned)->order_by('id', 'ASC');
        }
        $rows = $CI->db->get();
        if (!$rows) {
            return array();
...
if (!function_exists('resolveOutletScope')) {
    /**
     * Turn a posted outlet selection into a validated scope (Phase F).
     *
     * Accepts the literal string 'all' for the combined view, an outlet id, or
     * an empty value meaning "the outlet in session". A posted id the user has
     * no claim to is NOT honoured; it falls back to their session outlet.
     *
     * BASE OF A REXLIO FIX: Report::stockReport() previously took $_POST['outlet_id']
     * and passed it straight into the ledger with no check at all, so any user could
     * read any outlet's stock report by posting its id. That was introduced in the
     * Phase B stock report, not inherited from the vendor.
     *
     * @param  string $posted raw posted value
     * @return array{mode:string, ids:array, selected:string}
     */
    function resolveOutletScope($posted) {
        $CI = &get_instance();
        $accessible = getAccessibleOutletIds();
        $session_outlet = (int) $CI->session->userdata('outlet_id');

        if ($posted === 'all' && count($accessible) > 1) {
            return array('mode' => 'all', 'ids' => $accessible, 'selected' => 'all');
        }
        $posted_id = (int) $posted;
        if ($posted_id && in_array($posted_id, $accessible, TRUE)) {
            return array('mode' => 'single', 'ids' => array($posted_id), 'selected' => (string) $posted_id);
        }
        //fall back to the session outlet, but only if it is genuinely theirs
        if ($session_outlet && in_array($session_outlet, $accessible, TRUE)) {
            return array('mode' => 'single', 'ids' => array($session_outlet), 'selected' => (string) $session_outlet);
        }
        //last resort: first accessible outlet, or nothing at all
        if ($accessible) {
            return array('mode' => 'single', 'ids' => array($accessible[0]), 'selected' => (string) $accessible[0]);
        }
        return array('mode' => 'single', 'ids' => array(), 'selected' => '');
    }
}
if (!function_exists('outletScopeSqlList')) {
    /**
     * Render a scope's ids as a safe SQL IN list. Values are cast to int on the
     * way in by getAccessibleOutletIds(), and cast again here, so this cannot
     * carry anything but digits into the interpolated stock queries.
     *
     * @param  array $ids
     * @return string e.g. "1,2,4"; "0" when empty so the query matches nothing
     */
    function outletScopeSqlList($ids) {
        $clean = array();
...
if (!function_exists('outletScopeLabel')) {
    /**
     * Human label for the current scope, for report headers.
     * @return string
     */
    function outletScopeLabel($scope) {
        if (isset($scope['mode']) && $scope['mode'] == 'all') {
            return lang('all_outlets');
        }
        if (!empty($scope['ids'])) {
            return getOutletNameById($scope['ids'][0]);
        }
        return '';
    }
}
if (!function_exists('isSalesEnabledOutlet')) {
    /**
     * @param int $outlet_id defaults to the outlet in session
     * @return bool TRUE when orders may be taken at this outlet
     */
    function isSalesEnabledOutlet($outlet_id = '') {
        $CI = &get_instance();
        ...
        //Fail OPEN, deliberately. If the column is missing the query returns FALSE
        //(db_debug is off), and calling ->row() on that is itself fatal, so the
        //result is tested before it is used. A half applied migration must never
        //silently stop a live outlet from trading; the failure mode we want is
        //"guard not yet active", not "nobody can sell".
        $query = $CI->db->select('is_sales_enabled')->from('tbl_outlets')
                        ->where('id', $outlet_id)->where('del_status', 'Live')
                        ->get();
        if (!$query) {
            return TRUE;
        ...
        $outlet = $query->row();
        if (!isset($outlet->is_sales_enabled)) {
            return TRUE;
        }
