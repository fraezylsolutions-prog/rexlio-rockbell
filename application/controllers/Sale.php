...
class Sale extends Cl_Controller {
    ...
    public function __construct() {
        ...
        //Phase F: purchasing-only outlets (Central Store) must never open the sale
        //screen. Checked HERE, ahead of the register gate, on purpose: the register
        //gate below fires first otherwise, and the user is sent to open a register
        //at an outlet that does not sell - wrong message, and it invites opening a
        //register that should never exist.
        //
        //Scoped to the POS route rather than the whole controller because this
        //constructor also fronts push_online() and add_sale_by_ajax_split(), which
        //COMPLETE orders that already exist. Blocking those would strand an order
        //rather than prevent one. Placement is enforced separately and explicitly
        //in add_kitchen_sale_by_ajax().
        //
        //Exit path: Authentication::userProfile() bounces self order sessions and
        //role 'POS User' straight back to Sale/POS (Authentication.php:600-608,
        //:621), so redirecting those there loops forever. They get the terminal
        //message this controller already uses for a POS that must not open.
        $ir_route = $this->uri->segment(2);
        if(($ir_route=="POS" || $ir_route=="pos") && !isSalesEnabledOutlet($this->session->userdata('outlet_id'))){
            if($this->session->userdata('is_self_order')=="Yes" || $this->session->userdata('role')=='POS User'){
                echo "<h1 style='color:red;padding:10px;text-align:center'>".lang('outlet_sales_disabled')."</h1>";exit;
            }
            $this->session->set_flashdata('exception_er', lang('outlet_sales_disabled'));
            redirect('Authentication/userProfile');
        }

        //check register is open or not
        //
        //BASE-PRODUCT FIX: the vendor build excluded waiters from this check entirely
        //($designation!="Waiter" was part of this condition), so a waiter could open the
        //sale screen and place orders with no register open anywhere - sales outside any
        //tracked register session. Waiters are now checked too, but against a different
        //rule: a waiter never opens a register of their own, so requiring one would block
        //them permanently. Instead they are allowed to trade while ANY register is open at
        //their outlet. Cashiers/admins keep the original per-user check unchanged, including
        //its side effect of restoring counter/printer session data.
        //The food court and online-order exemptions are left exactly as the vendor had them.
        if($this->session->has_userdata('is_online_order')!="Yes" && !isFoodCourt()){
            $user_id = $this->session->userdata('user_id');
            $outlet_id = $this->session->userdata('outlet_id');
            if($designation=="Waiter"){
                $is_register_open = $this->Common_model->isAnyRegisterOpenForOutlet($outlet_id);
            }else{
                $is_register_open = $this->Common_model->isOpenRegister($user_id,$outlet_id);
            }
            if($is_register_open==0){
                //Sending someone to Register/openRegister only helps if they are
                //allowed to open one. A waiter is not, so bouncing them there would
                //land them on a "not permitted" error. Tell them to get a register
                //opened instead.
                if(!checkAccess("376", "open")){
                    $this->session->set_flashdata('exception_er', lang('register_not_open_ask_manager'));
                    redirect('Authentication/userProfile');
                }
    ...
    public function POS($user_id='', $outlet_id='',$sale_id=""){
        ...
        //Sales-disabled outlets are gated in __construct(), ahead of the register
        //check, so that the correct message is shown and no register detour happens.
        ...
        //Order attribution: a waiter may only ever attribute an order to themselves.
        //Filtered here rather than inside getWaitersForThisCompany() because that method
        //is also used by Setting.php (default waiter) and Waiter_app.php, which still
        //need the full list. Filtering server side means no other waiter's name or id
        //reaches the browser at all, so it cannot be re-enabled from dev tools.
        if(isWaiterUser()){
            $own_user_id = $this->session->userdata('user_id');
            $own_only = array();
            foreach($data['waiters'] as $single_waiter){
                if($single_waiter->id == $own_user_id){
                    $own_only[] = $single_waiter;
                }
            }
            $data['waiters'] = $own_only;
        }
        $data['is_waiter_user'] = isWaiterUser();
        $data['waiter_pin_required'] = waiterPinCheckRequired();
        ...
        //Price tiers. Only active tiers are offered on the sale screen; a tier that
        ...
        //
        //Computed here rather than client side because the arithmetic (ledger closing
        //balance minus committed, then the minimum across the recipe) needs eleven
        //ledger sources and the recipe expansion - none of which the browser has.
        //
        //Scoped to THIS outlet only, never All Outlets: what a counter can sell is
        //limited by what is at that counter's outlet, not by company wide totals.
        //This is also the corrected item 15 rule - Central Store running dry must not
        //stop a bar selling stock it already holds.
        //
        //NOTE this is a page load SNAPSHOT. The POS is offline first, so the figure
        //ages while the screen stays open. F5 will use it as the client side signal,
        //with the server check in add_kitchen_sale_by_ajax() as the real gate.
        $this->load->model('Inventory_model');
        $stock_outlet = array((int) $outlet_id);
        $on_hand = array();
        foreach($this->Inventory_model->getInventory('', '', '', $stock_outlet) as $stock_row){
            if(isset($stock_row->id) && $stock_row->id){
                $on_hand[$stock_row->id] = stockClosingBalance($stock_row);
            }
        }
        $committed = $this->Inventory_model->getCommittedStock($stock_outlet);
        $available_by_ingredient = array();
        foreach($on_hand as $ingredient_id => $qty){
            $available_by_ingredient[$ingredient_id] = $qty - (isset($committed[$ingredient_id]) ? $committed[$ingredient_id] : 0);
        }
        $requirements = $this->Inventory_model->getRecipeRequirements($company_id);
        //NULL means unconstrained (no recipe): F5 must read that as "never block",
        //not as zero, or every recipe-less item would be blocked outright.
        $data['available_units'] = array();
        foreach($requirements as $food_menu_id => $recipe){
            $data['available_units'][$food_menu_id] = maxSellableUnits($recipe, $available_by_ingredient);
        }
     ...
     */
    /**
     * Verify a waiter's login_pin before an order is attributed to them.
     * Called from the sale screen when a non-Admin picks a waiter other than themselves.
     * On success the waiter id is remembered in the session so the order-save path can
     * re-check it - see isWaiterAttributionAllowed().
     * @access public
     * @return void
     */
    public function verifyWaiterPinAjax(){
        $waiter_id = (int) $this->input->post('waiter_id');
        $pin = trim((string) $this->input->post('pin'));
        $response = array('status' => 0, 'msg' => lang('waiter_pin_incorrect'));

        if(!$waiter_id){
            echo json_encode($response);
            return;
        }
        //Admin never gets prompted, and attributing to yourself needs no PIN
        if(!waiterPinCheckRequired() || $waiter_id == (int) $this->session->userdata('user_id')){
            markWaiterPinVerified($waiter_id);
            $response['status'] = 1;
            $response['msg'] = '';
            echo json_encode($response);
            return;
        ...
        $waiter = $this->db->select('id, full_name, login_pin')
                           ->from('tbl_users')
                           ->where('id', $waiter_id)
        ...
        //A waiter with no PIN configured cannot be selected at all, rather than being
        //waved through - that would leave a hole exactly where this check exists.
        if($waiter->login_pin === NULL || trim($waiter->login_pin) === ''){
            $response['msg'] = lang('waiter_pin_not_set');
            echo json_encode($response);
            return;
        }
        if($pin !== '' && trim($waiter->login_pin) === $pin){
            markWaiterPinVerified($waiter_id);
            $response['status'] = 1;
            $response['msg'] = '';
        }
    ...
    public function add_kitchen_sale_by_ajax(){
        //BASE-PRODUCT FIX: refuse to write an order when no register is open.
        //The vendor build gated only the sale SCREEN, so a tab left open when a
        //register closed kept saving orders. Reuses the existing invoice_status
        //response shape, which the POS already surfaces as a toast error.
        if(!canPersistOrderNow()){
            $return_data = array();
            $return_data['invoice_status'] = '1';
            $return_data['invoice_msg'] = lang('register_closed_order_blocked');
            echo json_encode($return_data);
            return;
        }
        //Phase F: refuse to place an order at a purchasing-only outlet. Deliberately
        //placed on the PLACEMENT path only, not on push_online() or
        //add_sale_by_ajax_split() - those complete orders that already exist, and
        //blocking them would strand an order rather than prevent one. Same reasoning
        //as the register gate.
        if(!isSalesEnabledOutlet()){
            $return_data = array();
            $return_data['invoice_status'] = '1';
            $return_data['invoice_msg'] = lang('outlet_sales_disabled');
            echo json_encode($return_data);
            return;
        }
        //Phase F item 15: refuse to persist an order that oversells. The sale screen
        //already refuses to build such an order, but the screen can be stale, offline
        //or tampered with - same principle as the counter tier lock and the waiter PIN,
        //the UI asks and the server decides.
        //
        //Honest about the limit: push_online_for_kitchen() is fire and forget, so the
        //order is already in IndexedDB and on screen by the time this runs. Refusing
        //here stops it being PERSISTED; it cannot un-take the order. The client check
        //is the primary experience, this is the backstop.
        //
        //The sale_no is excluded from the committed figure because this endpoint also
        //handles MODIFICATIONS - without that, an existing order would be counted
        //against itself and even REDUCING its quantity would fail.
        if(isset($posted_order->items) && $posted_order->items){
            $stock_blocked = stockBlockedItems(
                $posted_order->items,
                $this->session->userdata('outlet_id'),
                $this->session->userdata('company_id'),
                isset($posted_order->sale_no) ? $posted_order->sale_no : ''
            );
            if($stock_blocked){
                $return_data = array();
                $return_data['invoice_status'] = '1';
                $return_data['invoice_msg'] = lang('stock_blocked_order').' '.implode(', ', $stock_blocked);
                echo json_encode($return_data);
                return;
            }
        }
        //Order attribution re-check. The sale screen prompts for the waiter's PIN, but
        //the posted payload cannot be trusted - swapping the waiter id in dev tools would
        //otherwise attribute an order to somebody who never authorised it. Same principle
        //as the counter tier lock: the UI asks, the server decides.
        $posted_order = json_decode(($this->input->post('order')));
        if(isset($posted_order->waiter_id) && !isWaiterAttributionAllowed($posted_order->waiter_id)){
            $return_data = array();
            $return_data['invoice_status'] = '1';
            $return_data['invoice_msg'] = lang('waiter_pin_required_for_attribution');
            echo json_encode($return_data);
            return;
        }
        ...
        if($status_creating_invocie==false){
            ...
            //price_tier travels with the order; defaults to Regular for payloads
            //created before this field existed.
            $data['price_tier'] = isset($order_details->price_tier) && $order_details->price_tier ? trim_checker($order_details->price_tier) : 1;
            //Counter lock enforcement. The sale screen already disables the buttons,
            //but the posted payload cannot be trusted, so pin both the tier and the
            //fulfilment type here for users without override rights. Returns 0 for
            //Admin/Manager and for counters left on Regular, in which case nothing changes.
            $locked_tier_for_order = getLockedPriceTier();
            if($locked_tier_for_order){
                $data['price_tier'] = $locked_tier_for_order;
                $data['order_type'] = orderTypeForPriceTier($locked_tier_for_order);
            }
            $this->db->trans_begin();
    ...
    public function push_online(){
        //DELIBERATELY NOT register-gated. This is the COMPLETION sync for an order
        //that already exists in the device's IndexedDB, not the creation of a new one.
        //Blocking it here would not prevent the sale - it already happened - it would
        //only strand it unsynced until a register reopened, creating a silent backlog.
        //New orders are stopped at the point of creation instead, in
        //add_kitchen_sale_by_ajax(). Late completions are allowed through on purpose.
        ...
        $data['order_type'] = trim_checker($order_details->order_type);
        $data['price_tier'] = isset($order_details->price_tier) && $order_details->price_tier ? trim_checker($order_details->price_tier) : 1;
        /* Does this request actually carry payment data?
           The completion sync (push_online / push_online_sync) posts the full
           order payload including payment_object. The EDIT path
           (push_online_update, pos_script_v7.3.js:8939) posts an order payload
           that contains no payment fields at all - no payment_object, no
           payment_method_type, no paid_amount, no due_amount. Everything below
           keys off this, because 'no payment data supplied' must mean 'leave the
           existing payment record alone', never 'assume Cash'. */
        $ir_has_payment_payload = isset($order_details->payment_object) && $order_details->payment_object;
        $this->db->trans_begin();
        if($sale_id>0){
            $data['modified'] = 'Yes';
            /* On the edit path these three were built from payload fields that do
               not exist, so trim_checker() returned '' and they would be written as
               0 - blanking the sale's real payment method and its paid/due figures.
               Drop them from the update so the stored values survive; paid_amount
               and due_amount are recomputed further down from the payment rows that
               actually exist. */
            if(!$ir_has_payment_payload){
                unset($data['payment_method_id'], $data['paid_amount'], $data['due_amount']);
            }
            $this->db->where('id', $sale_id);
            ...
            /* Payment rows are deliberately NOT deleted here. They are rewritten
               below, by the same writer the insert path uses, but only when this
               request actually supplies payment data. Deleting unconditionally here
               is what destroyed the real tender on every edit of a paid sale. */
            $sales_id = $sale_id;


            /* The hardcoded Cash row that used to be inserted here has been removed.
               It claimed payment_id=1/'Cash' for the full total_payable regardless of
               how the customer actually paid. getPaymentInfo() below only looks for
               the same payment_id, so it could never see or remove this row: a
        ...
        /* Clear before rewriting only when this request supplies the replacements.
           Without that condition an edit wipes the payment record and puts nothing
           back, which is worse than the bug being fixed. */
        if($ir_has_payment_payload){
        ...
        /* B1 routing. When no payment data was supplied the rows above were left
           untouched, so the real tender survives an edit. But if the edit changed
           the total, paid_amount and due_amount must be restated from the rows that
           actually exist, otherwise a shortfall is invisible. due_amount is the ONLY
           integration point for collection: Customer_due_receive_model::getCustomerDue()
           and every due report compute SUM(tbl_sales.due_amount) minus receipts, so
           writing it here is what routes the gap into the existing Customer Due
           Receive flow instead of fabricating a payment for it.

           No rescaling of the payment rows themselves - an edit is not a re-tender.
           If the edit LOWERED the total below what was paid, due_amount goes
           negative, which is a genuine customer credit and is what getCustomerDue()
           should see; the due reports filter on due_amount > 0 and ignore it. */
        if($sale_id>0 && !$ir_has_payment_payload){
            $ir_paid_row = $this->db->query(
                "SELECT COALESCE(SUM(amount),0) AS paid FROM tbl_sale_payments"
                ." WHERE sale_id=? AND del_status='Live'", array($sales_id))->row();
            $ir_paid = $ir_paid_row ? $ir_paid_row->paid : 0;
            $this->db->where('id', $sales_id);
            $this->db->update('tbl_sales', array(
                'paid_amount' => $ir_paid,
                'due_amount'  => $data['total_payable'] - $ir_paid,
            ));
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
