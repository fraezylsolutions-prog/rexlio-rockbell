...
class Common_model extends CI_Model {
    ...
    /**
     * Is ANY register currently open at this outlet.
     *
     * Used for waiters. A waiter never opens a register of their own (cashiers and
     * admins do), so the per-user isOpenRegister() check below would permanently
     * block them. The operational rule is that a waiter may take orders only while
     * the outlet is trading, i.e. while somebody has a register open.
     *
     * @access public
     * @return int number of open registers
     * @param int
     */
    public function isAnyRegisterOpenForOutlet($outlet_id){
        if(!$outlet_id){
            return 0;
        }
        $this->db->select('id');
     ...
     * @param array
     */
    public function getPreMadeIngredients($company_id,$status,$filters = array()) {
        $this->db->select('*');
        $this->db->from('tbl_ingredients');
        $this->db->where('company_id', $company_id);
        ...
        if (isset($filters['purchase_unit_id']) && $filters['purchase_unit_id'] !== '' && $filters['purchase_unit_id'] !== NULL) {
            $this->db->where('purchase_unit_id', $filters['purchase_unit_id']);
        }
        if (isset($filters['unit_id']) && $filters['unit_id'] !== '' && $filters['unit_id'] !== NULL) {
            $this->db->where('unit_id', $filters['unit_id']);
        }
        if (isset($filters['is_direct_food']) && $filters['is_direct_food'] !== '' && $filters['is_direct_food'] !== NULL) {
            $this->db->where('is_direct_food', $filters['is_direct_food']);
        }
        if (isset($filters['keyword']) && $filters['keyword'] !== '' && $filters['keyword'] !== NULL) {
            $this->db->group_start();
            $this->db->like('name', $filters['keyword']);
            $this->db->or_like('code', $filters['keyword']);
            $this->db->group_end();
        }
        $this->db->order_by('id',"DESC");
        return $this->db->get()->result();
    ...
    /**
     * Sale price of every food menu, keyed by food menu id (Phase F).
     *
     * Used by the item list to show a unit selling price for direct sale product
     * items, which carry their price on the food menu they were generated from
     * rather than on the item row. Fetched as one map instead of a per row lookup
     * so the list does not add an N+1 on top of the ones it already has.
     *
     * @access public
     * @return array food_menu_id => sale_price
     * @param  int $company_id
     */
    public function getFoodMenuSalePriceMap($company_id) {
        $this->db->select('id, sale_price');
        $this->db->from('tbl_food_menus');
        $this->db->where('company_id', $company_id);
        $this->db->where('del_status', 'Live');
        $rows = $this->db->get();
