...
class Outlet extends Cl_Controller {
        ...
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if(str_rot13($language_manifesto)=="eriutoeri"):
                    ...
                    //Phase F: purchasing-only outlets. Absent post means 1, so an
                    //outlet can never be silently disabled by a form that omits it.
                    $ir_sales_enabled = $this->input->post($this->security->xss_clean('is_sales_enabled'));
                    $outlet_info['is_sales_enabled'] = ($ir_sales_enabled === '0' || $ir_sales_enabled === 0) ? 0 : 1;
                endif;
    ...
    public function setOutletSession($encrypted_id, $then = '') {
        ...
        //that the user held enter-67, never that the outlet was one of theirs, and
        //Custom::encrypt_decrypt() returns its argument unchanged, so the id is not
        //obscured at all. Any user who could reach this route could enter any outlet
        //in the company by editing the URL. The outlet LIST is filtered
        //(Common_model::getAllOutlestByAssign) but the entry point was not, so the
        //filtering was cosmetic.
        if(!canEnterOutlet($outlet_id)){
            $this->session->set_flashdata('exception_er', lang('outlet_not_assigned'));
            redirect('Outlet/outlets');
        }
