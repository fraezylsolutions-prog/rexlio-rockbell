            ...
            <div class="box-body">
                <div class="row">
                    ...
                    <div class="col-md-6 col-lg-4 mb-3">
                        <div class="form-group">
                            <?php
                            /* See addCounter.php for the full note. Regular = unrestricted;
                               anything else locks the order-type row for ordinary staff at
                               this counter. Admin/Manager can still override. */
                            $current_tier = (int) (set_value('default_price_tier') ? set_value('default_price_tier') : (isset($counter->default_price_tier) ? $counter->default_price_tier : 1));
                            ?>
                            ...
                            <select name="default_price_tier" id="default_price_tier" class="select2 form-control">
                                <?php foreach($price_tiers as $tier): ?>
                                    <option <?php echo ((int) $tier->tier_key === $current_tier) ? 'selected' : ''; ?> value="<?php echo escape_output($tier->tier_key) ?>"><?php echo escape_output($tier->tier_name) ?></option>
                                <?php endforeach; ?>
                            </select>
