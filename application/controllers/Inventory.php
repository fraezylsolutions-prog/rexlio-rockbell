...
class Inventory extends Cl_Controller {
    ...
    public function index() {
        ...
        //Phase F: outlet scope. resolveOutletScope() validates the posted value
        //against the outlets this user may actually see, so a hand-crafted post
        //cannot read another outlet's stock.
        ...
        $data['inventory'] = $this->Inventory_model->getInventory($category_id, $ingredient_id, $food_id, $scope['ids']);
        //Phase F item 14: stock promised to placed but uncompleted orders. Scoped by
        //the same outlet selection as the on-hand figure above, so the two subtract
        //meaningfully - a per outlet Available, or a combined one under All Outlets.
        $data['committed'] = $this->Inventory_model->getCommittedStock($scope['ids']);
    ...
    public function getInventoryAlertList() {
        ...
        $data = array();
        //Phase F: same validated outlet scope as the inventory screen
        $scope = resolveOutletScope($this->input->post('outlet_scope'));
        $data['outlet_scope'] = $scope;
        $data['scope_outlets'] = getAllOutlestByAssign();
        $data['show_outlet_filter'] = count(getAccessibleOutletIds()) > 1;
        $data['inventory'] = $this->Inventory_model->getInventoryAlertList($scope['ids']);
